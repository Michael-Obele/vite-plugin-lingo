...
<div class="flex h-screen flex-col bg-slate-50">
	...
	<header class="flex items-center justify-between border-b border-slate-200 bg-white px-6 py-3 shadow-sm">
		<div class="flex items-center gap-3">
			<div class="flex items-center gap-2">
				...
				<h1 class="text-lg font-semibold text-slate-900">Lingo</h1>
			</div>
			<span class="text-sm text-slate-500">Translation Editor</span>
		</div>
	...
	<main class="flex flex-1 overflow-hidden">
		<!-- Sidebar -->
		<aside class="w-72 overflow-y-auto border-r border-slate-200 bg-white">
		...
		<section class="flex flex-1 flex-col overflow-hidden">
			{#if selectedLanguage}
				<!-- Filter tabs -->
				<div class="flex items-center gap-4 border-b border-slate-200 bg-white px-6 py-3">
			...
			{:else}
				...
				<div class="flex flex-1 items-center justify-center bg-slate-50">
					<div class="text-center">
						...
						<h2 class="mt-4 text-lg font-medium text-slate-900">Select a language</h2>
						<p class="mt-1 text-sm text-slate-500">Choose a language from the sidebar to start editing translations</p>
					</div>
				</div>
			{/if}
		</section>
