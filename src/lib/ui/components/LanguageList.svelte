...
<div class="flex h-full flex-col">
	...
	<div class="flex items-center justify-between border-b border-slate-200 px-4 py-3">
		<div class="flex items-center gap-2">
			...
			<h2 class="text-xs font-semibold uppercase tracking-wide text-slate-500">Languages</h2>
		</div>
		<button
			type="button"
			class="rounded-md p-1.5 text-slate-400 transition-colors hover:bg-slate-100 hover:text-slate-600 disabled:opacity-50"
	...
	{:else if languages.length === 0}
		<div class="flex flex-1 flex-col items-center justify-center gap-2 px-4 text-center text-slate-500">
			<FolderOpen class="h-8 w-8 text-slate-300" />
			<p class="text-sm font-medium">No .po files found</p>
			<p class="text-xs text-slate-400">Add .po files to your locales directory</p>
		</div>
	{:else}
				...
				<li>
					...
					>
						<div class="flex items-start justify-between">
							<div>
								<span class="block font-medium text-slate-900 {selected === lang.code ? 'text-blue-900' : ''}">
									{lang.name}
								</span>
								<span class="text-xs text-slate-500">{lang.code}</span>
							</div>
